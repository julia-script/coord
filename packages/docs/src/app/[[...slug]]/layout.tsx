...
function Breadcrumb({ path }: { path: RouteSummary[] }) {
  return (
      ...
      {path.map((item, index) => {
        ...
        return (
          <Fragment key={item.route + item.title}>
            {index > 0 && <ChevronRightIcon className="w-3.5 shrink-0" />}
            <div
              className={cn(
  ...
  return (
    ...
    >
      <main className=" w-full min-w-0 max-w-6xl px-6 pt-4 md:px-12">
        <Breadcrumb path={routeInfo.breadcrumbs} />

        <div className="prose prose-invert my-3 max-w-none">{children}</div>
        <div
          className={cn(
        ...
        >
          {previous && (
            ...
            >
              <ChevronRightIcon
                className={cn("inline h-5 shrink-0", "ltr:rotate-180")}
              />
              {previous.title}
            </Link>
          )}
...
export default function Layout(
  ...
  const route = props.params.slug?.join("/") ?? "";
  const { sidebar, header } = getProjectRoutes(route);
  const routeInfo = getRouteInfo(route);
  return (
    <>
      <style jsx global>{`
        :root {
          --accent-color: ${getAccentColor(route)};
        }
      `}</style>
      <Header items={header} />
      {routeInfo.layout !== "full" && (
        <div className={"mx-auto flex max-w-[90rem]"}>
          ...
          <Body routeInfo={routeInfo}>{props.children}</Body>
        </div>
      )}
      {routeInfo.layout === "full" && (
