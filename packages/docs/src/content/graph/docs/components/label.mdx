...
`Label` component allows you to attach a label with any valid JSX to a specific
position on a graph. The size of the label adapts to its children. You can also
adjust its position, orientation, distance from target, and color, among other
properties.
