...
Today, we are going to unlock the mysteries behind finding
the intersection between two lines.

Now, this might sound as if we are about to embark on a
journey to solve some formidable calculus problem, but fear
not!

We are merely diving into some delightful elementary
algebra. So let's get started, shall we?

## Setup

In our setup, we are going to draw two lines on a graph.
Line A - B goes from point A to point B. Similarly, Line C -
D travels from point C, to point D.
...
To find the slope, remember this: you need to know how much
your line goes up or down (the $y$-change) for each step it
takes to the left or right (the $x$-change).

In mathematics, this is beautifully represented by the
formula:
...
This, my dears, is the slope of _Line A - B_, denoted as
$m_{AB}$. Why we represent slopes with the letter $m$ is
beyound me, but it is what it is.

We calculate it by subtracting the $y$-coordinate of point A
from the $y$-coordinate of point B and dividing it by the
difference between the $x$-coordinates of point B and point
A.

> In plain english, the slope is the answer for the
> question: For each step horizontal step we take, how many
> steps do we take vertically?

Repeat this step for _Line C - D_ to find its slope.

### Show me the code

I know you're not here for the convoluted math formulas with
their single letter variable names, what are we? Savages?
...
Much better! We have a function that takes two points and
returns the slope of the line that connects them.
...
Vertical lines are a special case. Because they don't move
at all horizontally, they happen to cause a division by
zero. Which in JavaScript, gives us the special value
`Infinity`.
...
A line’s y-intercept is like a home base, the safe spot at y
when x equals zero. To find it, we employ the following
equation:
...
Where m is our previously calculated slope, y and x are the
coordinates of any point on the line, and b, our goal,
...
_Ever the romantic, I fancy the idea of two lines
intersecting at the horizon, somewhere far beyond our
reach._

To find this point, we pit the equations of our two lines
against each other, _et voila_, we have our $x$ coordinate.
Substituting $y$ into either line's equation reveals the $y$
coordinate.
...
With these, we can find the intersection point of the two
lines at infinity:
...
const findIntersection = (
  ...
  const x =
    (cdIntercept - abIntercept) / (abSlope - cdSlope);
...
export default function MyGraph() {
  ...
  // Intersection Point:
  const intersectionPoint = findIntersection(a, b, c, d);
  const isWithinAB = isWithinBoundingBox(
    intersectionPoint,
    a,
    b
  );
  const isWithinCD = isWithinBoundingBox(
    intersectionPoint,
    c,
    d
  );
  ...
  const intersectionColor = isWithinBoth
    ? "#16a34a"
    : "body";
  return (
    <Graph
...
While daydreaming of intersections at the end of the
universe can be delightful, sometimes we need to be grounded
in reality. The final act of our little escapade is
**checking if the intersection point** we found actually
**lies within the segments of the lines we're considering**.

To do that we can draw a square around our two lines and
check if the intersection point we found lies within those
squares.
...
maximum between between $$x_{A}$$ and $$x_{B}$$
respectively. The same applies for $$y_{min}$$ and
$$y_{max}$$.
...
And there we have it, darlings. A poetic journey into the
world of lines and their intersections. Mathematics, code,
and a touch of whimsy. Here's the final result:
