...
We also provide a `useCoordState` hook, that's just a wrapper on useState with a
few convienience features.

It automatically normalizes's any `Vec2ish` value passed as initial prop and
types it accordingly.

You could, however, just use `useState` and pass a `Vec2` directly if you
prefer.
...
Note that even in the ❌ cases, the Marker will still probably work, but the
`point` state will change to a Vec2 after the first `onChange` callback So if
you're using Typescript, it will complain about the type mismatch.

You could solve make it type safe again by doing something like this. (is it
worth it though? 🤔)
...
For finer control, you can also leverage other HTML native events, such as
`onMouse*`, `onPointer*`, etc.
