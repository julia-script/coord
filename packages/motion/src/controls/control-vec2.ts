...
export function controlVec2(getter: () => Vec2, setter: (value: Vec2) => void) {
  return new Control(getter, setter);
}

export function* makeVec2State<TKey extends string>(
  key: TKey,
  initialState: Vec2ish
) {
  const control = yield* makeState(key, Vec2.of(initialState));

  return new Vec2Control(control._get, control._set);
}
