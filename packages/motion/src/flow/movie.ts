...
import { InferThread, Thread, Threadish, normalizeThreadsSet } from "@/utils";
...
>(key: TKey, scene: TScene, transition = 0) {
  ...
  context.meta.start = parentContext.frames.length;
  parentContext.meta.scenes[key] = context.meta;
  const firstFrame = context.frames[0]!;
  const sceneState = yield* makeState<TKey, typeof firstFrame>(key, firstFrame);
  ...
  sceneState.set((curr) => ({ ...curr, $done: true }));
}

export function movie<TThread extends Thread[]>(...threads: TThread) {
  return function* () {
    const activeScenes = new Set<TThread[number]>();
    const startNext = () => {
