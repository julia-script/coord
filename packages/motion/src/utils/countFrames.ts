export const countFrames = (time: number, fps: number, duration: number) => {
  ...
  let remainder = expectedFinishTime - actualFinishTime;

  if (remainder >= 1 / fps) {
    frames++;
    remainder = expectedFinishTime - (time + frames / fps);
  }
